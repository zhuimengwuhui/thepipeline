...
class Stages(object):
    ...
    def snp_recalibrate_gatk(self, genotype_vcf_in, outputs):
        ...
        gatk_args = "-T VariantRecalibrator --disable_auto_index_creation_and_locking_when_reading_rods " \
                    ...
                    "-rscriptFile {snp_plots} -mode SNP".format(reference=self.reference, cores=cores,
                    one_k_g_snps=self.known_dog_snps, genotype_vcf=genotype_vcf_in, recal_snp=recal_snp_out,
                    tranches_snp=tranches_snp_out, snp_plots=snp_plots_r_out)
